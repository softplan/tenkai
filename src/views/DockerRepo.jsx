...
class DockerRepo extends Component {
  ...
  getRepositories() {
    axios.get(TENKAI_API_URL + '/dockerRepo')
  ...
  onConfirmDelete(item) {
    axios.delete(TENKAI_API_URL + "/dockerRepo/" + item.name)
      .then(res => {
        this.getRepositories();
      }).catch(error => {
        console.log(error.message);
  ...
  handleConfirmDelete() {
    if (this.state.itemToDelete !== undefined) {
      axios.delete(TENKAI_API_URL + "/dockerRepo/" + this.state.itemToDelete.name)
        .then(res => {
          this.getRepositories();
        }).catch(error => {
          console.log(error.message);
  ...
  onSaveClick(data) {
      ...
      axios.post(TENKAI_API_URL + "/dockerRepo", data)
        .then(res => {
          ...
          <Row>
            <Col md={12}>
              {this.state.showInsertUpdateForm ?
                <DockerRepoForm editMode={this.state.editMode} editItem={this.state.editItem} saveClick={this.onSaveClick.bind(this)} cancelClick={this.handleCancelEnvironmentClick.bind(this)} /> : null
              }
            </Col>
          </Row>
