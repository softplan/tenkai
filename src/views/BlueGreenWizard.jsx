...
class Step1 extends React.Component {
  ...
  updateProps() {
    ...
    if (state !== undefined) {
      ...
      let onlyChartName =  state.chartName.substring(state.chartName.indexOf("/")+1);
      let serviceName = onlyChartName + '-' + this.props.envName;
      this.props.updateServiceName(serviceName);
      let defaultReleaseName = state.releaseName.substring(0, state.releaseName.indexOf("-beta")) + "-" + NAMESPACE;
  ...
  async componentDidMount() {
    await this.refs["h1"].getVariables(this.props.chartName, this.props.chartVersion);
  }
    ...
    return(
      <div>
        ...
        <HelmVariables 
          ...
          key="h1" chartName={this.props.chartName} chartVersion={this.props.chartVersion}
          xref={"h1"}
          ref={"h1"}
...
class BlueGreenWizard extends Component {
  ...
  constructor(props) {
    ...
    if (charts !== undefined && charts.length > 0) {
      ...
      let chartName = "saj6/" + firstElement.substring(0, firstElement.indexOf("@"));
      let chartVersion = firstElement.substring(firstElement.indexOf("@") + 1, firstElement.length);
  ...
  get nextButton(){
    ...
    return (
      ...
      <button 
        className="btn btn-primary pull-right" 
        type="button" onClick={this.finishWithCanary.bind(this)}>
        Finish with Canary
      </button>        

      <button 
        className="btn btn-primary pull-right" 
        type="button" onClick={this.finishWithBlueGreen.bind(this)}>
  ...
  render() {
    ...
    let firstClassName = this.state.currentStep  === 1 ? "btn btn-success btn-circle btn-fill" : "btn btn-default btn-circle";
    let secondClassName = this.state.currentStep  === 2 ? "btn btn-success btn-circle btn-fill" : "btn btn-default btn-circle";
                        ...
                        <div className="stepwizard">
                            <div className="setup-panel">
                                <div className="stepwizard-step col-xs-3"> 
                                    <a href="#step-1" type="button" className={firstClassName} disabled={this.state.currentStep!==1}>1</a>
                                    <p><small>Configure a new version of service</small></p>
                                </div>
                                <div className="stepwizard-step col-xs-3"> 
                                    <a href="#step-2" type="button" className={secondClassName} disabled={this.state.currentStep!==2}>2</a>
                                    <p><small>Configure traffic rules</small></p>
                                </div>
                            </div>
                        </div>
                    ...
                    content={
                      <div>
                          ...
                          <Step1
                            ...
                            updateDefaultReleaseName={this.updateDefaultReleaseName.bind(this)}
                            updateHeaderReleaseName={this.updateHeaderReleaseName.bind(this)}
