...
function deleteSolutionChart(id, self) {
    if (self.state.itemToDelete !== undefined) {
        axios.delete(TENKAI_API_URL + "/solutionCharts/" + id)
            .then(retrieveSolutionChart(self.state.solutionId, self)).catch(error => {
                console.log(error.message);
                self.props.handleNotification("general_fail", "error");
            });
    }
...
export {
    retrieveSolutionChart, saveSolutionChart, deleteSolutionChart
};